...
def assign_id(existing_id):
    id_list = []
    id_list = [x for x in range(1000) if x not in existing_id and x !=0]
    return min(id_list)
...
class OpParamsFrm(Frame):
    ...
    def add_new_params(self):
        ...
        for psf in self.p_subfrm[1:len(self.p_subfrm)]:
            idx_duprd.extend([i for (i, rdop) in enumerate(self.op_params['route_ops'])
                              if rdop['route_id'] == psf.params_temp['route_id']
                              and rdop['direction'] == psf.params_temp['direction']])
        ...
        if idx_dupcid is not None and idx_duprd is not None:  # update the route operation mode
            ...
            dfc['case_id'] = assign_id(self.csni_list[1])
            dfc['simulated'] = 0
            dfc['p_filename'] = ''

            for f, r in zip(self.p_subfrm[1:len(self.p_subfrm)], self.df_rd_ops):
                if idx is None:
                    rdop = copy.deepcopy(r)
                    rdop.update(f.params_temp)
            ...
            for f, r, idx in zip(self.p_subfrm[1:len(self.p_subfrm)], self.df_rd_ops, idx_duprd):
                if idx is None:
                    dfc = copy.deepcopy(r)
                    dfc.update(f.params_temp)
                    self.op_params['route_opd'].append(dfc)
