...
def sim():
    ...
    idx_def = next((index for (index, d) in enumerate(system.op_params['op_cases']) if
                    d['case_id'] == system.op_params['df_case_id']), None)
...
class SimCaseFrm(Frame):
    def __init__(self, *d):  # d[0]: root, d[1]: system
        ...
        self.case_df = self.system.op_params['df_case_id']
...
class RailSystemMenu(Menu):
    ...
    def _load_sim_case(self):
        ...
        def run_sim():
            ...
            self.system.op_params['df_case_id'] = self.system.op_params['op_cases'][c_idx]['case_id']
            self.system.gen4sim()
...
class NetParamsFrm(Frame):
    ...
    def __init__(self, *d):  # d[0]: root, d[1]: system
        ...
        idx_c = next(
            (i for (i, c) in enumerate(self.op_params['op_cases']) if c['case_id'] == self.op_params['df_case_id']), None)
        self.df_case = self.op_params['op_cases'][idx_c]
        self.df_rdop_id = self.df_case['rd_ops']
        # df_rdop_id = [rdop_id1, rdop_id2, rdop_id3, ...]; rdop: route_id, direction, op_parameters
    ...
    def add_new_params(self):
        ...
        if idx_dup is not None:  # duplicated route id and direction
            ...
            params_temp['route_id'] = assign_id(self.rni_list[1])
            self.df_route = params_temp
...
class OpParamsFrm(Frame):
    ...
    def show_params(self):
        ...
        self.p_subfrm.append(ParamsFrm(self.p_frm, self.df_case, [sheet_c1]))

        sheet_c2 = [0, ['Route ID', 'route_id', 'int', 10],
                    ['Train Type', 'train_type', 'int', 10], ['Power Net ID', 'pwrnet_id', 'int', 10]]

        self.frm_rtp = Frame(self.p_frm)
        self.frm_rtp.pack(side=TOP)
        self.p_subfrm.append(self.frm_rtp)
        self.var_comb_r = StringVar()
        self.comb_r = ttk.Combobox(self.frm_rtp, textvariable=self.var_comb_r, values=list(self.rni_list[0]))
        self.comb_r.pack(side=LEFT, padx=10)
        self.var_comb_rid = IntVar()
        self.comb_rid = ttk.Combobox(self.frm_rtp, textvariable=self.var_comb_rid, values=list(self.rni_list[1]))
        self.comb_rid.pack(side=LEFT, padx=10)
        self.comb_r.bind('<<ComboboxSelected>>', self.net_entry)

        cidx_r = next((i for (i, r) in enumerate(self.rni_list[1]) if int(r) == self.df_rd_ops[0]['route_id']), None)
        self.comb_r.current(cidx_r)
        self.comb_rid.current(self.comb_r.current())

        self.var_comb_t = StringVar()
        self.comb_t = ttk.Combobox(self.frm_rtp, textvariable=self.var_comb_t, values=list(self.tti_list[0]))
        self.comb_t.pack(side=LEFT, padx=10)
        cidx_t = next((i for (i, t) in enumerate(self.tti_list[1]) if int(t) == self.df_rd_ops[0]['train_type']), None)
        self.comb_t.current(cidx_t)

        self.var_comb_p = StringVar()
        self.comb_p = ttk.Combobox(self.frm_rtp, textvariable=self.var_comb_p, values=list(self.pni_list[0]))
        self.comb_p.pack(side=LEFT, padx=10)
        self.comb_p.bind('<<ComboboxSelected>>', self.pwr_entry)
        cidx_p = next((i for (i, p) in enumerate(self.pni_list[1]) if int(p) == self.df_rd_ops[0]['pwrnet_id']), None)
        self.comb_p.current(cidx_p)

        # sheet_c3 = [31, 'Route Direction', 'direction', ['Upward', '1', 'int', 10],
        #             ['Downward', '2', 'int', 10]]
        ...
        for dc in self.df_rd_ops:
            layout = [sheet_c2, sheet_c4, sheet_c5, sheet_c6, sheet_c7]
            if dc['direction'] == 0:
                strDir = ' Upward'
            elif dc['direction'] == 1:
                strDir = ' Downward'
            p_frm_rd = ParamsFrm(self.p_ntbk, dc, layout)
            self.p_ntbk.add(p_frm_rd, text='Route ' + str(dc['route_id']) + strDir)
            self.p_subfrm.append(p_frm_rd)
        self.p_ntbk.select(0)

    def pwr_entry(self, event):
        pew_id = self.pni_list[1][self.comb_p.current()]
        for frm in self.p_subfrm[2:len(self.p_subfrm)]:
            ent = next((v for (k, v) in zip(list(frm.key_ent.keys()), list(frm.key_ent.values())) if k == 'pwrnet_id'), None)
            if ent is not None:
                ent.set(pew_id)

    def net_entry(self, event):
        self.comb_rid.current(self.comb_r.current())
        net_id = self.rni_list[1][self.comb_r.current()]
        for frm in self.p_subfrm[2:len(self.p_subfrm)]:
            ent = next((v for (k, v) in zip(list(frm.key_ent.keys()), list(frm.key_ent.values())) if k == 'route_id'), None)
            if ent is not None:
                ent.set(net_id)

    def tr_entry(self, event):
        tr_id = self.tti_list[1][self.comb_t.current()]
        for frm in self.p_subfrm[2:len(self.p_subfrm)]:
            ent = next((v for (k, v) in zip(list(frm.key_ent.keys()), list(frm.key_ent.values())) if k == 'train_type'), None)
            if ent is not None:
                ent.set(tr_id)
    ...
    def add_new_params(self):
        ...
        dfc = self.p_subfrm[0].params_temp
        dfc['case_id'] = assign_id(list(self.csni_list[1]))
        dfc['simulated'] = 0
        dfc['p_filename'] = ''
        dfc['op_mode'] = self.var_rdir.get()
        dfc['rd_ops'] = []
        for f, dfrd in zip(self.p_subfrm[1:len(self.p_subfrm)], self.df_rd_ops):
            rdop = f.params_temp
            rdop['rdop_id'] = assign_id(list(self.rdop_id_list))
            rdop['direction'] = dfrd['direction']
            dfc['rd_ops'].append(rdop['rdop_id'])
    ...
    def save_params_as(self, f_name):
        ...
        self.op_params['op_cases'][self.idx_dfc]['simulated'] = 0
        self.op_params['op_cases'][self.idx_dfc]['p_filename'] = ''

        for f, rdop, idx in zip(self.p_subfrm[1:len(self.p_subfrm)], self.df_rd_ops, self.idx_rd):
            f._save_temp()
            rdop.update(f.params_temp)
            self.op_params['route_ops'][idx].update(rdop)
    ...
    def list_case(self):
        ...
        self.var_rdir = IntVar(self.master)
        self.var_rdir.set(self.df_case['op_mode'])
        self.rb_updir = Radiobutton(self.p_selfrm, text='Upward ', variable=self.var_rdir, value=0,
                                    command=_list_rdop)
        self.rb_updir.pack(side=LEFT, padx=5, pady=10)
        self.rb_dndir = Radiobutton(self.p_selfrm, text='Downward ', variable=self.var_rdir, value=1,
                                    command=_list_rdop)
        self.rb_dndir.pack(side=LEFT, padx=5, pady=10)

        self.rb_dndir = Radiobutton(self.p_selfrm, text='Cycling ', variable=self.var_rdir, value=2,
                                    command=_list_rdop)
        self.rb_dndir.pack(side=LEFT, padx=5, pady=10)
