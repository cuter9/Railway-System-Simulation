...
class SysTemplate:
    ...
    def __net_params_temp__(self):
        route_1 = {'route_id': 1,
                   ...
                   'len': 6.3,  # km
                   'no_station': 9,
                   # 'loc_station': [0.195, 1.078, 1.813, 2.334, 2.975, 3.907, 4.789, 5.665, 6.270, 6.45],
                   # 'name_station': ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B11', 'B12', 'YE'],  # km
                   'stations': [[0.195, 1.078, 1.813, 2.334, 2.975, 3.907, 4.789, 5.665, 6.270, 6.45],
                                ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B11', 'B12', 'YE']],  # km
        ...
        route_2 = {'route_id': 2,
                   ...
                   'len': 6.8,  # km
                   'no_station': 10,
                   # 'loc_station': [0.195, 1.078, 1.813, 2.334, 2.975, 3.907, 4.789, 5.481, 6.098, 6.703, 6.80],
                   # 'name_station': ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B9', 'B10', 'YE'],  # km
                   'stations': [[0.195, 1.078, 1.813, 2.334, 2.975, 3.907, 4.789, 5.481, 6.098, 6.703, 6.80],
                                ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B9', 'B10', 'YE']],  # km
...
class System:
    ...
    def run_simulation(self, msg_txt):
        ...
        r_idx = next((idx for (idx, r) in enumerate(net_params['routes']) if r['route_id'] == r_id), None)
        ...
        if self.net4sim.op_params['op_mode'] == 0:  # whole day timetable
            if sim_params['disturbance'][0] == 1 and sim_params['disturbance'][1] == 0:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                           + 'r' + str(self.net4sim.op_params['route_id']) + '_whole_day_time_table_1_dd'
            elif sim_params['disturbance'][0] == 0 and sim_params['disturbance'][1] == 1:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                           + 'r' + str(self.net4sim.op_params['route_id']) + '_whole_day_time_table_1_da'
            elif sim_params['disturbance'][0] == 1 and sim_params['disturbance'][1] == 1:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                           + 'r' + str(self.net4sim.op_params['route_id']) + '_whole_day_time_table_1_d'
            else:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                           + 'r' + str(self.net4sim.op_params['route_id']) + '_whole_day_time_table_1'
        elif self.net4sim.op_params['op_mode'] == 1:
            if np.all((sim_params['disturbance'] == 0)):
                d = ''
            else:
                d = '_d'
            prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                       + 'r' + str(self.net4sim.op_params['route_id']) + 'h' \
                       + str(self.net4sim.op_params['time_tbl'][1][0]) + 'd' \
                       + str(self.net4sim.op_params['dwell_time']) + 'o' \
                       + str(self.net4sim.op_params['time_tbl'][0][0]) + d
        ...
        self.op_params['op_cases'][c_idx]['p_filename'] = self.op_params_df['p_filename']      # set default op case simulated
...
def plot_train_resistance():
    tr_params = System('Train').tr_params
