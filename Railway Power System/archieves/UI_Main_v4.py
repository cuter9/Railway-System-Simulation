...
class OpParamsFrm(Frame):
    ...
    def show_params(self):
        ...
        sheet_c11 = [31, 'Travel Direction', 'op_mode', ['Upward', '0', 'int', 10],
                     ['Downward', '1', 'int', 10], ['Cycling', '2', 'int', 10]]
        self.p_subfrm.append(ParamsFrm(self.p_frm, self.df_case, [sheet_c11]))
        ...
        def tab_chg(event):
            # self.show_params()
            tab_sel = self.p_ntbk.tab(self.p_ntbk.select())
            # tab_sel.

        self.p_ntbk = ttk.Notebook(self.p_frm, padding=[5, 5, 5, 5])
        self.p_ntbk.pack(side=TOP, fill=X)
        # self.p_ntbk.bind('<<NotebookTabChanged>>', tab_chg)
        ...
        # self.p_frm_rd_add = Frame(self.p_ntbk)
        # self.p_frm_rd_add.pack()
        # self.p_ntbk.add(self.p_frm_rd_add, text='+')
        # self.p_subfrm.append(self.p_frm_rd_add)
    ...
    def save_params_as(self, f_name):
        ...
        self.df_case['op_mode'] = self.var_rdir.get()
