...
class system():
    def __init__(self, sys_type):
        ...
        if sys_type == 'New':
            self.__system_gen__()
        elif sys_type == 'Network':
            self.__network_gen__()
        elif sys_type == 'Train':
            self.__train_gen__()
        elif sys_type == 'Power':
            self.__pwr_system_gen__()
        elif sys_type == 'Operation':
            self.__operation_gen__()
        elif sys_type == 'Simulation':
            self.__simulation_gen__()

    def __system_gen__(self):
        self.__network_gen__()
        self.__train_gen__()
        self.__pwr_system_gen__()
        self.__operation_gen__()
    ...
    def __pwr_system_gen__(self):
        pwr_params = {'no_BSS': 2,
                      ...
                      'volt_TSS': 750,  # Volt
                      'range_TSS': array([[0, 1.8], [1.8, 2.9], [2.9, 4.8], [4.7, self.net_params['routes'][0]['len']]]),
                      # [kp, kp]
        ...
        self.pwr_params = pwr_params

    def __operation_gen__(self):
    ...
    def __run_simulation__(self):
        ...
        net_params_r = net_params['routes'][r_id]
        system = net.Network(net_params_r, tr_params, pwr_params, op_params, sim_params)
        ...
        if system.op_params['headway'][0] == -1:
