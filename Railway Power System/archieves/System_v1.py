...
class System:
    ...
    def run_simulation(self, msg_txt):
        ...
        if r_dir == 3:
            ...
            self.net_params_df_up = net_params['routes'][r_idx_up]
            self.net_params_df = self.net_params_df_up
            self.net4sim_up = net.Network(self)

            r_idx_down = next((idx for (idx, r) in enumerate(net_params['routes'])
                        if r['route_id'] == r_id and r['direction'] == 1), None)
            self.net_params_df_down = net_params['routes'][r_idx_down]
            self.net_params_df = self.net_params_df_down
            self.net4sim_down = net.Network(self)
        ...
        if self.net4sim.op_params['op_mode'] == 0:  # whole day timetable
            if sim_params['disturbance'][0] == 1 and sim_params['disturbance'][1] == 0:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                            + 'r' + str(self.net4sim.op_params['route_id']) \
                            + 't' + str(self.net4sim.op_params['direction']) \
                            + '_whole_day_time_table_1_dd'
            elif sim_params['disturbance'][0] == 0 and sim_params['disturbance'][1] == 1:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                            + 'r' + str(self.net4sim.op_params['route_id']) \
                            + 't' + str(self.net4sim.op_params['direction']) \
                            + '_whole_day_time_table_1_da'
            elif sim_params['disturbance'][0] == 1 and sim_params['disturbance'][1] == 1:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                            + 'r' + str(self.net4sim.op_params['route_id']) \
                            + 't' + str(self.net4sim.op_params['direction']) \
                            + '_whole_day_time_table_1_d'
            else:
                prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
            ...
            prof_name = 'c' + str(self.net4sim.op_params['case_id']) \
                        + 'r' + str(self.net4sim.op_params['route_id']) \
                        + 't' + str(self.net4sim.op_params['direction']) \
                        + 'h' + str(self.net4sim.op_params['time_tbl'][1][0]) \
                        + 'd' + str(self.net4sim.op_params['dwell_time']) \
                        + 'o' + str(self.net4sim.op_params['time_tbl'][0][0]) + d
