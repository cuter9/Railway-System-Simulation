...
class Network:
    ...
    def init_network(self):
        ...
        self.pwr_profile = [[self.sim_params['t_op']], [0]]  # [t, p]
        for r in self.df_case['route_id']:
            tr_type = self.df_case['train_type']
            ...
            for rdop in self.df_rd_ops:
                if rdop['route_id'] == r:
                    ...
                    idx_net = next((i for (i, n) in enumerate(self.net_params['routes'])
                                    if n['route_id'] == rdop['route_id'] and n['direction'] == rdop['direction']), None)
                    self.net_params_rd = self.net_params['routes'][idx_net]

                    self.sim_params_rd = self.sim_params

                    route = Route(self)
                    self.trips_info['no_active_trips'] += route.trips_info['no_active_trips']
                    # self.trips_info['current state'] = self.trips_info['current state'] \
                    #                                   + '( Route ' + str(rdop['route_id']) + ', Direction ' \
                    #                                   + str(rdop['direction']) + ' ) ' \
                    #                                   + route.trips_info['current state'] + '\n'
                    self.routes.append(route)

        self.trips_info['trips_changed'] = 1
    ...
    def next_state(self):
        ...
        for r in self.routes:
            ...
            r.__next_state__()
            if r.trips_info['trips_changed'] == 1:
                self.trips_info['trips_changed'] = 1
                self.trips_info['current state'] = self.trips_info['current state'] \
                                                   + '( Route ' + str(r.net_params['route_id']) \
                                                   + ', Direction ' + str(r.net_params['direction']) + ' ) ' \
                                                   + r.trips_info['current state'] + '\n'
            self.trips_info['no_active_trips'] += r.trips_info['no_active_trips']

            self.instant_pwr = self.instant_pwr + r.instant_pwr

        self.pwr_profile = np.concatenate((self.pwr_profile, [[self.sim_params['t_op']], [self.instant_pwr]]), axis=1)


class Route:

    def __init__(self, system):
        self.net_params = system.net_params_rd
        self.tr_params = system.tr_params_rd
        self.pwr_params = system.pwr_params_rd
        self.op_params = system.op_params_rd
        self.sim_params = system.sim_params_rd
        ...
        self.trips_info = {}
        self.trips_info['no_active_trips'] = 0
        # self.trips_info['trips_changed'] = 0
        self.active_trips = []
    ...
    def __gen_time_table__(self):
        ...
        self.op_params['depart_time'] = [self.op_params['depart_offset']]
        # ldt = 0  # last_depart_time
        ldt = self.op_params['depart_offset']  # last_depart_time
        ...
        for trip in self.active_trips:
            if trip['active'] == 1:
                ...
                if self.pwr_params['pwr_reg'] == 1 or \
                    ...
                    self.instant_pwr = self.instant_pwr + trip['train'].state[
                        'power']  # accumulate the power of each active trip

        self.pwr_net.__whole_pwr__(self.instant_pwr, t_op + ts)
    ...
    def __update_trip__(self, trip):  # update the nth active trip state for the next time step
        ...
        if train.state['control'] == 0:  # not need to check constraint if train is in dwell state
            ...
            if self.sim_params['acc_disturb'] == 0:
    ...
    def check_constraint(self, train):  # check conflict with speed limit
        ...
        if np.size(brk_prof[0]) != 0 and np.size(p_st) != 0:
            if brk_prof[0][-1] >= st[p_next]:
                ...
                if self.sim_params['dwell_disturb'] == 1:
